...
def initiate():
    ...
    # Bulk create CarMake instances
    car_make_instances = CarMake.objects.bulk_create([
        CarMake(name=data['name'], description=data['description'], country=data['country'])
        for data in car_make_data
    ])
    
    print(f"Created {len(car_make_instances)} CarMakes.")
    ...
    # Bulk create CarModel instances
    CarModel.objects.bulk_create([
        CarModel(car_make=data['car_make'], name=data['name'], type=data['type'], year=data['year'])
